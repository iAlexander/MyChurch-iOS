...
- (NSArray *)shortMonthNames {
    if (!_shortMonthNames) {
        NSMutableArray *temp = [NSMutableArray arrayWithArray:[self.dateFormatter monthSymbols]];
        for (int i = 0; i < [temp count]; i++)
        {
            NSString *month = [temp objectAtIndex:i];
            month = [NSString stringWithFormat:@"%@%@",[[month substringToIndex:1] uppercaseString],[month substringFromIndex:1] ];
            [temp replaceObjectAtIndex:i withObject:month];
        }
        _shortMonthNames = temp;
    }
...
- (void)setDateFormatTemplate:(NSString *)dateFormatTemplate {
    ...
    NSString* dateFormat = [NSDateFormatter dateFormatFromTemplate:dateFormatTemplate options:0 locale:[[NSLocale alloc] initWithLocaleIdentifier:@"uk_UA"]];
...
- (NSDateFormatter *)dateFormatter {
    if (!_dateFormatter) {
        ...
        _dateFormatter.locale = [[NSLocale alloc] initWithLocaleIdentifier:@"uk_UA"];
        [_dateFormatter setDateFormat:[NSDateFormatter dateFormatFromTemplate:self.dateFormatTemplate options:0 locale:[[NSLocale alloc] initWithLocaleIdentifier:@"uk_UA"]]];
