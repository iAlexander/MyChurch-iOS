...
class Label: UILabel {
    ...
    func setAttributedText(_ string: String?, withColor color: UIColor? = nil) {
        ...
        self.attributedText = NSMutableAttributedString(string: string, attributes: [self.custom.foregroundColor: color, custom.kern: self.kern!, custom.font: UIFont.systemFont(ofSize: self.size, weight: self.weight), custom.strikethroughStyle: self.strikethroughStyle!, custom.paragraphStyle: self.paragraphStyle])
    }
